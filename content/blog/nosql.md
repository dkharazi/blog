...
At a high level, the CAP theorem states we can't choose two of the above metrics without sacrificing the third metric. A partition refers to an interruption in communication within a distributed system. Meaning, partition tolerance indicates a cluster must continue to work despite any breakdown in communication. Consistency implies that each node, including replica nodes, maintains the same data. Availability means a request will return a response, even if nodes are down. For more information about the CAP theorem, refer to [this IBM article](https://www.ibm.com/cloud/learn/cap-theorem).
